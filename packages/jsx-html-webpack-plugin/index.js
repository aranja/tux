...
class JsxHtmlWebpackPlugin {
  constructor(options) {
    ...
    if (typeof this.options.document !== 'string') {
      throw new Error('JsxHtmlWebpackPlugin requires a file with a jsx Document.')
    }

    const htmlPluginOptions = Object.assign({
      template: `${require.resolve('./lib/loader')}!${require.resolve('./lib/renderer')}?${this.options.document}`,
      inject: false,
