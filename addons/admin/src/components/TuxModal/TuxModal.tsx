    ...
    return (
      InputComponent && (
        <div className="InputComponent" key={field.field}>
          <label className="InputComponent-label ">{field.label}</label>
          <InputComponent
            id={field.field}
            onChange={(state: any) => this.onChange(state, field.field)}
            value={value}
            {...field.props}
          />
          <style jsx>{`
          ...
          <form onSubmit={this.onSubmit}>
            <div className="TuxModal-topBar">
              <h1 className="TuxModal-title">
                {modalHeading}{' '}
                <strong className="TuxModal-modelName">{meta.name}</strong>
              </h1>
              <div className="TuxModal-buttons">
                <Button onClick={this.onCancel}>Cancel</Button>
                <Button type="submit" themeColor="green" raised>
                  {modalAction}
                </Button>
              </div>
            </div>
